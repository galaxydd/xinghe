...
def process_csv(input_file:str ,output_file: str):
    ...
    # 随机筛选条地方性法规
    selected_local = random.sample(local_regulations, min(44, len(local_regulations)))
    # 合并所有需要处理的数据
    # 法律5条  行政法规 1条  地方性法规44条
    all_data = laws + admin_regulations + selected_local
    print(f"总计生成 {len(all_data)} 条尚未生效的的法律条文")
    # 构建JSON结构
    json_data = []
...
if __name__ == "__main__":
    file_path = './result'
    input_file = os.path.join(file_path,'invalid.csv')
    process_csv(input_file,'./result_json/invalid.json')
