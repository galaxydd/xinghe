...
from model_gen import model_generator, get_fewshot_examples, process_prompt, truncate_long, ACCURACY_FILES
...
class Chatlaw_generator(model_generator):
    def __init__(self, is_few_shot, device, is_vllm, model_path, model_name, few_shot_path):
        ...
        '''
        super(Chatlaw_generator, self).__init__(is_few_shot, device, is_vllm, model_path, few_shot_path)
        self.model_name = model_name
        
    def model_init(self):
        tokenizer = AutoTokenizer.from_pretrained(self.model_path, trust_remote_code=True)
        model = AutoModelForCausalLM.from_pretrained(self.model_path, torch_dtype=torch.float16, device_map="auto", trust_remote_code=True).eval()
